...
def metodaRelaksacjiGlobalnej(wG):
    ...
    while 1:
        ...
        for i in range(1,nx):
            for j in range(1,ny):
                err = ( (Vn[i+1][j] - 2*Vn[i][j] + Vn[i-1][j]) / (delta**2) + (Vn[i][j+1] - 2*Vn[i][j] + Vn[i][j-1]) / (delta**2) ) + p[i][j] / epsilon
        ...
        for i in range(nx):
            for j in range(ny):
                S += delta**2 * ( 1/2. * ((Vn[i+1][j] - Vn[i][j]) / delta)**2 + 1/2. * ((Vn[i][j+1] - Vn[i][j]) / delta)**2 - p[i][j] * Vn[i][j] )
